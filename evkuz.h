 ...
 *
 * 11.11.2019
 * Сигнал нажатия кнопки "Очистить экран" поместил в слот обработки нажатия кнопки "Показать RealTime" \
 * on_Show_Selected_Spectral_Single_pushButton_clicked()
 * Теперь для отображения графика другой станции надо выбрать нужный номер станции и нажать кнопку "Показать RealTime".
 * Здесь, в тестовом проекте данные в буфер попадают из файла порцией по 8 байт <<File_read_Current(byInputBuffer);>>.
 * В реальном рабочем проекте в этом буфере всегда будет много больше байт.
 * Фиксируем результат, сохраняем коммит.
 * 299e176
 * 299e176f224de53dac9f9e68a106012f1be5e694
 *
 * Дальнейшие задачи.
 * - Увеличить скорость накопления спектра
 * - Сделать файл с данными для нескольких каналов (данные) на одну станцию
 * - Сделать обработку остальных типов команд (остальные виды блоков)
 * - Сделать возможность отображения спектра для заданного канала в многоканальных блоках
 *
 * Скорость увеличили.
 * Считываем файл по 1840 байт - это 10 точек для каждой станции. Также увеличил таймаут для потока до 100мс.
 * В итоге за 1 сек. (timer_2) мы имеем 10 срабатываний запуска потока, и при каждом срабатывании получаем 10 точек для
 * заданной станции, т.е. 100точек/сек. Уменьшил таймер до 50мс, получаем 200точек/сек.
 * Сохраняем коммит.
 *
 *
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 08.11.2019
 * Файл создал(проект ADP_Test_Gen), - ADP_data_000.bin, - теперь надо его гонять по кругу и выдавать данные в буфер.
 *
 * Данные кладутся в файл для станции начиная с 1 и с канала 100, далее для каждого номера станции увеличиваем на 100 номер канала.
 *
 * Т.е. надо создавать дополнительный поток, для работы с этим файлом.
 * ...
 * А вот и нет. Т.к. считывание по  USB происходит как раз в потоке А, то и считывание из файла (вместо считывания из USB) будем
 * делать в потоке А.
 *
 * Вот на этом этапе очень помогло наличие схемы "Накопление в потоке" - за 5 минут все понял.
 *
 * Заработало !!!
 *
 * Теперь обходит файл, где данные для всех станций. Т.е теперь для одной станции данные накапливаются в 23 раза реже.
 * НАдо ускорить работу потока.
 * Сохраняем комммит с текущей скоростью(QThread::usleep(1000);), а в следующем уже увеличим.
 * ec4332d103e368614e839f5d9e18116d5f01833f
 *
 * И надо понимать, что теперь 17-я станция попадает в 1700 канал.
 * Теперь надо поправить смену N_number при изменения номера в списке прокуртки.
 *
 *
 * //+++++++++++++++++++++++++++++++++
 * 07.11.2019
 * Ранее при нажатии кнопки "ПОКАЗАТЬ Real Time" в класс Plot передавался номер команды (b0, b2), который
 * не вычислялся из принятых данных (так правильно), а был прописан "руками" (так неправильно, но было нужно для тестов).
 *
 * И вот это теперь, на данном этапе пора переделывать, т.к. мы же берем все данные от конкретной станции, а что там за команда и как эти данные
 * интерпретировать зависит от протокола, т.е. от самих данных.
 *
 * Тогда следует определять тип команды в самой ф-ции Plot::Draw_Spectral_N_Chan_Slot.
 * Сделано.
 *
 * Теперь В принятой порции из 8 байт определяется номер станции, определяется тип команды.
 * Далее в зависимости от типа команды идет выборка байт с данными для отрисовки (4, 5 для b0/b2 и 2-7 для b1)
 *
 * Следующий шаг - направлять в буфер USB данные из файла, где информация от разных станций.
 * Такой файл будет создавать проект ADP_Test_Gen. Проект пока не закончен, ждем его завершения.
 * А пока фиксируем состояние, сохраняем коммит.
 * a3e600fec5fe08d66dc103b9b804fcd0901c2201
 * Следующим шагом делаем доработку класса Plot.
 *
 * У нас класс Plot объявляется в файле graph.h, что, наверное, не совсем правильно.
 * Более правильно объявить класс Plot в файле plot.h, а весь функционал оставить в файле graph.cpp
 * Сделано. Сохраняем коммит
 * 8307b13a9528fb564bbfef31dec26c77a1874fd2
 *
 *
 * //+++++++++++++++++++++++++++++++++
 * 06.11.2019
 * ЗАРАБОТАЛО !!!
 * - Гистограмма по тестовым данным отображается.
 * - Работаем без участия железа
 * - Отображаем только для 1 станции (№17)
 *
 * Сохраняем коммит.
 * c453507547160f45e6feef036e12c526786039d7
 *
 * Добавил очистку графика и отрисовку шкалы в исходное состояние при нажатии кнопки "ОЧИСТИТЬ ОКНО".
 * Кнопка "ОЧИСТИТЬ ОКНО" сделана для упрощения отработки перерисовки графика при смене номера станции.
 * Спрятал код перерисовки гистограммы в класс Plot, для инкапсуляции.
 * Так удобнее будет работать с классом Plot в других проектах
 * Сохраняем коммит.
 * 11b1d205ec6fe22fd9837d598d9eb980a40166a1
 * Следующим шагом надо задать массив данных для всех станций камак, чтобы можно было выбрать
 * любую станцию и получить гистограмму. При этом текущий график очищается, массивы обнуляются.
 * Можно сделать большой массив для всех 23 станций, но тогда надо как-то перемещаться по этому массиву.
 * Поищем что-нибудь про кольцевой буфер в Qt
 *
 * Решено, напишу консольное приложение для создания двоичного файла данных.
 * Будет несколько параметров :
 *
 * 1. Стартовый канал для станции
 * 2. Количество каналов
 * 3. Интервал между каналами (100-200-500 рандомизировать)
 * 4. Очередность следования записей для станции(рандомизировать)
 *
 * Нам помогут ранее созданные проекты check_consistensy + repair_data_files
 * Кстати надо их залить на git
 *
 * Консольный проект называется ADP_Test_Gen.
 *
 * //+++++++++++++++++++++++++++++++++
 * 05.11.2019
 * Ветка hardware_off_01
 * Добавляем функционал, позволяющий делать отрисовку спектров без подключенного оборудования.
 * Т.е. чисто имитируем данные и на их основе рисуем спектры.
 *
 * - Для начала отключаем все возможные проверки подключения чипа ft2232h.
 *   А это подразумевает :
 *   - Read_A->Не удалось очистить Rx буфер FTDI
 *   - Неизвестный код возврата от ftdi_init()
 *   - Read_AAA()->FT_GetQueueStatus()->В буфере нет данных для считывания, RxBytes == 0, пропускаем
 *   - В "FTDI_D2XX::Read_A" делаем подмену данных:
 *       ftStatus = FT_OK;
 *       memcpy(byInputBuffer, TEST_DATA, 8);
 *     Где буфер TEST_DATA хранит фиксированные 8 байт от станции №17


 *
 * Все ведет к весьма значительным переделкам кода
 *
 * //+++++++++++++++++++++++++++++++++++++++++
 * 05.11.2019
 * С этой точки начинаем добавлять функционал, специфичный для ADP
 *
 * Предстоит сделать функционал для отрисовки спектра в выбранном канале блока (до 16 каналов на блок, номер канала - это субадрес КАМАК).
 * Для этого предстоит имитировать входные данные. Ранее эту задачу делало fpga, теперь же решено это сделать программным способом.
 * Таким образом возникает следующая задача :
 * - Создать область памяти - общую для 2 процессов (потоков). 1 поток пишет в эту область данные, а другой считывает
 *   Размер этой области - 0x400000 // это 4 194 304 байт !
 *
 * Закомментировано копирование массива byOutputBuffer в массив OutputBuffer, почему ? Как теперь данные передаются ?
 * Данные пишутся(append) в массив, на который смотрит указатель BUF_PTR, а таких массивов 2.
 *
 * - Доработал FTDI_D2XX::Read_AAA(void)
 * - Доработал FTDI_D2XX::Read_A()
 * - ДОбавил QThread::usleep(500);
 *
 ...
 *   05.11.2019
 *
 * Закомментировал void on_Show_Spectral_Selected_From_File_Single_pushButton_clicked();
 * строка 278 mainwindow.h
 *
 * Закомментировал обращение к кнопке "Show_Spectral_Selected_From_File_Single_pushButton" т.к. она вообще убрана из интерфейса
 * строка 56 mainwindow.cpp
 *
 * Сохраняем коммит, используем дальше как отправная точка для работы с ADP
 *
 * //+++++++++++++++++++++++++++++++++++++++++
 * 21.06.2019
 *
 * Нигде нет описания, как работает отрисовка. Блок-схема на visio не очень информативна.
 *
 * - Сделать кнопку на вкладке "Single", по нажатию которой будет очищаться буфер точек для отображения спектра
 *   Это нужно чтобы убрать "мусор" с экрана (очистить экран) и начать отрисовку спектра заново
 * - ДОбавил кнопку и слоты
 ...
 * //++++++++++++++++++++++++
 * 20.06.2019
 * - Т.к. сократил имя файла с "Camac_Thread_data_000.bin" на "Camac_data_000.bin", то и
 *   смена индекса файла должна поменяться. Вот поменял.
 * - Поправил ошибку при нажатии "Stop ALL". camac_tab->KK_connected был сразу false, что неверно, т.к. девайс на связи остался, мы лишь
 *   только остановили набор.
 * - Подправил счетчик выполненных команд из файла
 * - Подправил вывод завершающего сообщения после обработки командного файла.
 *
 * //++++++++++++++++++++++++++++
 * 19.06.2019
 ...
 * Сохраняю коммит... 455bcab
 *
 * -  Убрал groupbox код команды КАМАК
 * Сохраняю коммит... 7a4755e
 *
 * - Исправил название файла, теперь "Camac_data_xxx.bin"
 * - Исправил название главного окна, теперь вместо "MainWindow" Значится "KK_USB_01"
 *
 * - Закомментировал строки 255-258 в mainwindow.cpp
 * - Закомментировал строки 1318, 1322  в mainwindow.cpp
 *
 * //++++++++++++++++++++++++
 * 18.06.2019
